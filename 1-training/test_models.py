...
class ModelTester:
    ...
    def load_models(self):
        ...
        if not os.path.exists(MODELS_DIR):
            raise FileNotFoundError(f"Models directory '{MODELS_DIR}' not found. Run train.py first.")
        ...
        for filename in model_files:
            ...
            if not os.path.exists(filepath):
                raise FileNotFoundError(f"Required file '{filename}' not found in models directory.")
    ...
    def get_passenger_info(self):
        ...
        # Basic information
        try:
            passenger_data['pclass'] = int(input("Passenger Class (1=First, 2=Second, 3=Third): "))
            ...
            embarked_input = input("Embarked (C=Cherbourg, Q=Queenstown, S=Southampton): ").upper().strip()
    ...
    def predict(self, passenger_data):
        ...
        try:
            ...
            print(f"  Prediction: {'SURVIVED' if lr_proba >= 0.5 else 'DID NOT SURVIVE'}")
            
            print("\nDecision Tree:")
            print(f"  Survival Probability: {dt_proba:.3f} ({dt_proba*100:.1f}%)")
            print(f"  Prediction: {'SURVIVED' if dt_proba >= 0.5 else 'DID NOT SURVIVE'}")
            
            print("\n🎯 ENSEMBLE PREDICTION:")
            print(f"  Survival Probability: {ensemble_proba:.3f} ({ensemble_proba*100:.1f}%)")
            print(f"  Final Prediction: {'✅ SURVIVED' if ensemble_pred else '❌ DID NOT SURVIVE'}")
    ...
    def display_passenger_summary(self, passenger_data):
        ...
        print("-" * 60)
        print(f"Class: {passenger_data['pclass']} ({'First' if passenger_data['pclass']==1 else 'Second' if passenger_data['pclass']==2 else 'Third'})")
        print(f"Sex: {passenger_data['sex'].title()}")
        print(f"Age: {passenger_data['age']}")
        print(f"Family: {passenger_data['sibsp']} siblings/spouses, {passenger_data['parch']} parents/children")

...
class TitanicPreprocessor:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._is_fitted:
            raise RuntimeError("Preprocessor must be fitted before transforming new data. Call fit_transform() first.")
    ...
    def _preprocess(self, df: pd.DataFrame, is_training: bool = True) -> pd.DataFrame:
        ...
        if is_training:
            self.preprocessing_stats = {
                ...
                'survival_rate': df['survived'].mean() if 'survived' in df.columns else None
            }
        ...
        if is_training:
            self.feature_columns = [col for col in df_processed.columns if col != 'survived']
    ...
    def _handle_missing_values(self, df: pd.DataFrame, is_training: bool) -> None:
        ...
        if 'embarked' in df.columns:
            ...
            fill_value = self.preprocessing_stats.get('embarked_mode', df['embarked'].mode()[0])
        ...
        if 'fare' in df.columns:
            ...
            fill_value = self.preprocessing_stats.get('fare_median', df['fare'].median())
                ...
                if is_training:
                    ...
                    if col in self.label_encoders:
                        ...
                        try:
                            ...
                            print(f"Warning: Unseen category in {col}. Using most frequent value.")
                            # For unseen categories, use the most frequent training category
                            mode_category = self.label_encoders[col].classes_[0]
                            df[col] = df[col].map(lambda x: x if x in self.label_encoders[col].classes_ else mode_category)
                            df[col] = self.label_encoders[col].transform(df[col])
    ...
    def get_feature_columns(self) -> List[str]:
        ...
        """
        if not self._is_fitted:
            raise RuntimeError("Preprocessor must be fitted before accessing feature columns.")
    ...
    def load_artifacts(cls, models_dir: str) -> 'TitanicPreprocessor':
        ...
        if not os.path.exists(encoders_path):
            raise FileNotFoundError(f"Required file 'label_encoders.pkl' not found in {models_dir}")
        ...
        if not os.path.exists(stats_path):
            raise FileNotFoundError(f"Required file 'preprocessing_stats.json' not found in {models_dir}")
        ...
        if not os.path.exists(features_path):
            raise FileNotFoundError(f"Required file 'feature_columns.json' not found in {models_dir}")
    ...
    def preprocess_single_passenger(self, passenger_data: Dict) -> pd.DataFrame:
        ...
        """
        if not self._is_fitted:
            raise RuntimeError("Preprocessor must be fitted before preprocessing passenger data.")
...
def create_passenger_dataframe(pclass: int, sex: str, age: float, sibsp: int, 
                             parch: int, fare: float, embarked: str) -> pd.DataFrame:
    """

...
class RateLimiterConfig:
    ...
    def _ensure_initialized(self):
        ...
        if not self._initialized and config_manager.config:
            self.config = config_manager.config
            # Override with config values if available
            self.default_rate_limit = getattr(self.config.api, 'rate_limit', '100/minute')
    ...
    def get_redis_url(self) -> Optional[str]:
        ...
        Returns:
            str or None: Redis URL if available, None for in-memory storage
        """
        self._ensure_initialized()
        # For development, use in-memory storage
        # For production, configure Redis URL via environment variable
        import os
...
# Create limiter instance with in-memory storage for now
limiter = Limiter(

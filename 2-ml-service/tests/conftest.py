...
async def mock_ml_service(mock_models_dir):
    ...
    with patch('app.services.ml_service.ml_service') as mock_service:
        ...
        mock_service.is_healthy = Mock(return_value={
            "status": "healthy",
            "models_loaded": True,
        ...
        mock_service.get_feature_columns = Mock(return_value=[
...
def app_client(mock_config, mock_ml_service):
    ...
    with patch('main.app_config', mock_config), \
...
def health_checker_mock():
    ...
    mock_checker.run_startup_checks = AsyncMock(return_value={

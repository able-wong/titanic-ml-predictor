...
def authenticated_request(app_client, mock_ml_service=True):
    ...
    if mock_ml_service:
        with patch('app.api.routes.predictions.ml_service') as mock_service:
            ...
            mock_service.predict_survival = AsyncMock(return_value={
                "individual_models": {
                    "logistic_regression": {"probability": 0.888, "prediction": "survived"},
...
class TestPredictionEndpoints:
    ...
    def test_prediction_with_invalid_token(self, app_client, valid_passenger_data):
        ...
        response = app_client.post("/predict", json=valid_passenger_data, headers=headers)
    ...
    def test_prediction_input_validation(self, app_client, mock_auth_headers, invalid_passenger_data):
        ...
        with authenticated_request(app_client, mock_ml_service=False):
            for invalid_data in invalid_passenger_data:
                response = app_client.post("/predict", json=invalid_data, headers=mock_auth_headers)
    ...
    def test_prediction_enhanced_validation(self, app_client, mock_auth_headers):
        ...
        with authenticated_request(app_client, mock_ml_service=False):
            ...
            response = app_client.post("/predict", json=malicious_data, headers=mock_auth_headers)
    ...
    def test_prediction_anomaly_detection(self, app_client, mock_auth_headers):
        ...
        with self.authenticated_request(app_client), \
            ...
            response = app_client.post("/predict", json=anomalous_data, headers=mock_auth_headers)
...
class TestErrorHandling:
    ...
    def test_ml_service_error_handling(self, app_client, mock_auth_headers, valid_passenger_data):
        """Test ML service error handling."""
        with authenticated_request(app_client, mock_ml_service=False):
            with patch('app.api.routes.predictions.ml_service') as mock_service:
                # Mock ML service failure
                from app.core.exceptions import PredictionError
                mock_service.predict_survival = AsyncMock(side_effect=PredictionError("Model prediction failed"))
    ...
    def test_specific_error_status_codes(self, app_client, mock_auth_headers, valid_passenger_data):
        ...
        with authenticated_request(app_client, mock_ml_service=False):
            ...
            with patch('app.api.routes.predictions.ml_service') as mock_service:
                ...
                mock_service.predict_survival = AsyncMock(side_effect=PredictionInputError("Invalid input data"))
            ...
            with patch('app.api.routes.predictions.ml_service') as mock_service:
                ...
                mock_service.predict_survival = AsyncMock(side_effect=ModelUnavailableError("Models not available"))
...
class TestRateLimiting:
    ...
    def test_prediction_rate_limiting(self, app_client, mock_auth_headers, valid_passenger_data):
